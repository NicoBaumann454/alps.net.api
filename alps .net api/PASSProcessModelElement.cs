...
namespace alps.net_api
{
    ...
    public class PASSProcessModelElement : OwlThing, IPASSProcessModellElement
    {
        ...
        /// <summary>
        /// Method that exports the classes to a file
        /// </summary>
        /// <param name="g"></param>
        /// <returns></returns>
        public virtual void export(ref Graph g)
        {
            IUriNode subject;
            IUriNode predicate;
            ILiteralNode objec;
            ...
            string nameString = modelComponentID;
            Uri name = new Uri(nameString);

            subject = g.CreateUriNode(name);
            predicate = g.CreateUriNode("rdf:type");
            objec = g.CreateLiteralNode("owl:NamedIndividual");
            ...
            string tmp = this.GetType().ToString().Split('.')[2];

            objec = g.CreateLiteralNode("standard-pass-ont:" + tmp);

            test = new Triple(subject, predicate, objec);
            g.Assert(test);

            tmp = getModelComponentID().Split('#')[1];
            //tmp = tmp + "^^http://www.w3.org/2001/XMLSchema#string";
            subject = g.CreateUriNode(name);
            predicate = g.CreateUriNode("standard-pass-ont:hasModelComponentID");
            objec = g.CreateLiteralNode(tmp);

            test = new Triple(subject, predicate, objec);
            g.Assert(test);
            //Console.WriteLine(subject.ToString() + "    " + predicate.ToString() + "    " + objec.ToString());

            foreach (string s in modelComponentLabel)
            {
                subject = g.CreateUriNode(name);
                predicate = g.CreateUriNode("standard-pass-ont:hasModelComponentLabel");
                objec = g.CreateLiteralNode(s);
            ...
            {
                subject = g.CreateUriNode(name);
                predicate = g.CreateUriNode("rdf:comment");
                objec = g.CreateLiteralNode(s);
            ...
            foreach (string s in getAdditionalAttribute())
            {
                ...
                {
                    string[] split = getAdditionalAttributeType()[counter].Split('#');
                    type = split[1];
                    type = "standard-pass-ont:" + type;
                }
                else
                {

                    if (getAdditionalAttributeType()[counter].Contains("abstract-pass-ont"))
                    {
                        string[] split = getAdditionalAttributeType()[counter].Split('#');
                        type = split[1];
                        type = "abstract-pass-ont:" + type;
                    }
                }

                value = getAdditionalAttribute()[counter];

                //if (s.Contains("^"))
                //{
                //string[] split = getAdditionalAttribute()[counter].Split('^');
                //value = split[0];
                //}

                subject = g.CreateUriNode(name);
                predicate = g.CreateUriNode(type);
                objec = g.CreateLiteralNode(value);
